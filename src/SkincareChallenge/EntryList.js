...
function EntryList() {
  ...
  const getUsers = async (perPage=10, pageNo=1) => {
    try {
      ...
      const response = await fetch(`${config.SKINCAREBASEURL}/api/challenge/all-entries?perPage=${perPage}&pageNo=${pageNo}&orderBy=entries.id`,requestOptions);
    ...
    <PageWrapper>
        ...
        <EntryTable>
          <table>
            ...
            {entries && entries.length > 1 && <tbody>
              {entries.map((entry, i) => {
                return (
                  <tr key={i} id="row">
                    <td>{entry.id}</td>
                    ...
                    <td><Link to = {{ pathname: `/Challenge/Entry/${entry.id}`, state: entry }}>Edit</Link></td>
                  </tr>
                )
              })}              
              </tbody>
