...
useEffect(() => {
    ...
    <PopUp style={showDelete ? {display: 'block'} : {display:'none'}}><Model type='entry' close={handlePopUp} delete={handleDelete} text="Deleting this entry will delete all submissions associated with the entry as well. This action can not be undone." /></PopUp>
            ...
            <Col>
              ...
              <div>
                ...
                <div className="read-only-value" style={{marginTop:'5px'}}>
                      <div  className='check'>
                        <input type="checkbox" disabled={!edit} checked={entry.goalWeight} />
                             <span>Weight Management Support</span>
                      </div>
                      <div className='check'>
                        <input type="checkbox" disabled={!edit} checked={entry.goalImmune} />
                              <span>Promote a Healthy Immune System</span>
                      </div>
                      <div className='check'>
                        <input type="checkbox" disabled={!edit} checked={entry.goalStress} />
                            <span>Help with Situational Stress</span>
                      </div>
                      <div className='check'>
                        <input type="checkbox" disabled={!edit} checked={entry.goalSleep} />
                              <span>Aid Sleep During Restless Night</span>
                      </div>
                      <div className='check'>
                        <input type="checkbox" disabled={!edit} checked={entry.goalOverall} />
            ...
            <Col>
              ...
              <div style={{border: '1px solid #707070', padding:'1px', marginTop:'5px'}}>
               <GrayBox>
                   ...
                   <div className='check' >
                        <input type="checkbox" id='check' disabled={!edit} checked={entry.height !== ''} />
                               <span>Height</span>
                      </div>
                      <div  className='check'>
                        <input type="checkbox" disabled={!edit} checked={entry.gender !== ''} />
                               <span>Gender</span>
                      </div>
               </GrayBox>
       ...
       { edit && <div style={{display:'flex', flexDirection:'row', justifyContent:'right'}}><Delete onClick={ handlePopUp}>Delete Entry</Delete></div>}
        </EntryDetails>
      </div>
    );
  }
